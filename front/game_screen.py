...
class GameScreen(Screen):
    ...
    def setup_game(self, surface: pygame.Surface, stats_panel_visibility: bool = True):
        ...
        self.render_manager = RenderManager(
            surface=surface, debug=self.debug, game=self.game
        )
        ...
        if event.type == pygame.KEYDOWN:
            if pygame.key.get_mods() & pygame.KMOD_SHIFT:
                if event.key == pygame.K_1:
                    ...
                    self.game.spawn_enemy(EnemyType.GHOST)
                    ...
                    self.game.add_entity(
                        ArtifactChest(
                            Vector2(pygame.mouse.get_pos()), Shrapnel(self.game.player)
                        )
                    )
                    ...
                    self.game.add_entity(
                        ArtifactChest(
                            ...
                            BulletShield(self.game.player),
                        )
                    )
                elif event.key == pygame.K_SPACE:
                    self.game.add_entity(
                        ArtifactChest(
                            Vector2(pygame.mouse.get_pos()), MineSpawn(self.game.player)
                        )
                    )
                elif event.key == pygame.K_d:
                    self.game.add_entity(
                        ArtifactChest(
                            Vector2(pygame.mouse.get_pos()), Dash(self.game.player)
                        )
                    )
                elif event.key == pygame.K_r:
                    self.game.add_entity(
                        ArtifactChest(
                            Vector2(pygame.mouse.get_pos()), Rage(self.game.player)
                        )
                    )
    ...
    def update(self, time_delta: float):
        ...
        if self.game.paused:
            self.paused_label.update()
        if (
            self.game.collected_artifact_cache
        ):  # this adds the artifact to the ult picker
            self.render_manager.ult_picker.add_artifact(
                self.game.collected_artifact_cache.pop()
            )
    ...
    def spawn_notification(
        ...
        if at_pos == "player":
            pos = self.game.player.get_pos() + random_vector
        elif at_pos == "cursor":
            pos = Vector2(pygame.mouse.get_pos()) + random_vector
        elif at_pos == "center":
            pos = self.surface.get_rect().center + random_vector
        else:
            pos = at_pos + random_vector
        self.notifications.append(
            Notification(
                text=text,
                position=pos,
    ...
    def post_run(self):
        ...
        if self.game.time < SAVE_GAMES_LONGER_THAN:
            return
